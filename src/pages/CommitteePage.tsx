...
const Section = ({ title, children }) => (
  <section className="scroll-mt-24">
    <div className="flex items-center gap-4 mb-10 pb-4 border-b border-[#E5E7EB]">
      <ScrollText className="w-7 h-7 text-[#C9AB63]" />
...
const CommitteeList = ({ members }) => (
  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
    {members.map((member, index) => (
      <div
        key={index}
        className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 text-[15px]"
      >
        <span className="font-semibold text-[#1F2327]">
          {member.name}
        </span>
        
        <span className="hidden sm:inline text-[#C9AB63]">•</span>
        <span className="text-[#6B6F74]">
          {member.affiliation}
        </span>
      </div>
    ))}
